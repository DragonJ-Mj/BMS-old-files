...
exports.webp = async function (message) {
  for (let i = 0; i < queueWebp.length; i++) {
    if (queueWebp[i].from == message.from) {
      console.log(queueWebp[i].from)
      sendSticker.convertSticker(queueWebp[i])
      queueWebp.splice(i, 1)
    }
  }
}
